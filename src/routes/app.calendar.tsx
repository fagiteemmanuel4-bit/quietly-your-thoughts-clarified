    ...
    (async () => {
      try {
        const snap = await getDocs(
          query(collection(db, "users", user.uid, "events"), orderBy("date", "asc")),
        );
  ...
  return (
    <div className="min-h-screen">
          ...
          {cells.map((d, i) => {
            ...
            return (
              <button
                ...
                className={`aspect-square md:aspect-[5/4] rounded-md border bg-card paper-lift p-1.5 md:p-2 text-left flex flex-col gap-1 ${isToday ? "border-foreground" : "border-border"}`}
              >
                ...
                <div className="flex-1 space-y-0.5 overflow-hidden">
                  {todays.slice(0, 2).map((e) => (
                    <div key={e.id} className="text-[10px] truncate chip-sage rounded px-1 py-0.5">
                      {e.time ? `${e.time} ` : ""}
                      {e.title}
                    </div>
                  ))}
      ...
      {open && (
        ...
        >
          <div
            className="w-full max-w-md bg-card rounded-md border border-border p-5 scale-fade-in"
            onClick={(e) => e.stopPropagation()}
          >
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-2">
              {new Date(open).toDateString()}
            </p>
            ...
            <div className="space-y-2">
              <input
                ...
                className="w-full px-3 py-2 rounded-md border border-border bg-transparent text-sm outline-none focus:border-ring"
              />
              <input
                value={time}
              ...
              <div className="flex gap-2 pt-2">
                <Button onClick={create} disabled={!title.trim()} className="flex-1">
