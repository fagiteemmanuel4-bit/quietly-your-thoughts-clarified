      ...
      <div className="border-b border-border/60 px-4 md:px-10 py-5 flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Move things forward, one calm step at a time.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex border border-border rounded-md overflow-hidden">
            <button
              ...
              className={`px-2.5 py-1.5 text-xs flex items-center gap-1 ${view === "kanban" ? "bg-foreground text-background" : "text-muted-foreground"}`}
            >
              <LayoutGrid className="h-3 w-3" /> Kanban
            </button>
            <button
              onClick={() => setView("list")}
              className={`px-2.5 py-1.5 text-xs flex items-center gap-1 ${view === "list" ? "bg-foreground text-background" : "text-muted-foreground"}`}
            >
          ...
          <Button
            ...
            size="sm"
            variant="outline"
            className="border-violet/40 text-violet hover:bg-violet-soft"
          >
              ...
              <div key={c.id} className="rounded-md border border-border bg-card paper-lift">
                <div className="flex items-center justify-between px-4 py-3 border-b border-border/60">
                  <h2 className="text-sm font-medium">
                    {c.label} <span className="text-muted-foreground">· {byCol(c.id).length}</span>
                  </h2>
                  <button
                    onClick={() => {
                      setAdding(c.id);
                ...
                <div className="p-3 space-y-2 min-h-[180px]">
                  {adding === c.id && (
                    <form
                      ...
                      className="rounded border border-ring bg-card p-2"
                    >
                      <input
                        value={draft}
                        autoFocus
                  ...
                  {byCol(c.id).map((t) => (
                    <div
                      key={t.id}
                      className="group rounded border border-border/60 p-2.5 hover:border-foreground/30 transition"
                    >
                      <div className="flex items-start gap-2">
                        ...
                        />
                        <p
                          className={`text-sm flex-1 ${t.done ? "line-through text-muted-foreground" : "text-foreground/90"}`}
                        >
                          {t.title}
                        </p>
                        <button
          ...
          <div className="rounded-md border border-border bg-card divide-y divide-border/60">
            ...
            {tasks.map((t) => (
              <div key={t.id} className="flex items-center gap-3 px-4 py-3 group">
                ...
                <span
                  className={`text-[10px] uppercase tracking-wider rounded-full px-2 py-0.5 ${t.priority === "urgent" ? "chip-violet" : "chip-sage"}`}
                >
                  {t.priority}
                </span>
                <button
