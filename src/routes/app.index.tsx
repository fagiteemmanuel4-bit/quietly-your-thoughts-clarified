...
function Dashboard() {
  ...
  const generateBrief = async () => {
    ...
    try {
      const recent = thoughts.slice(0, 5).map(t => t.output || t.text || t.input || "").filter(Boolean);
      const res = await brief({ data: { name: user.displayName?.split(" ")[0] || undefined, thoughts: recent } });
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-border/60 px-4 md:px-10 py-5">
        <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">{new Date().toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric" })}</p>
        ...
        <div className="lg:col-span-2 rounded-md border border-border bg-card paper-lift p-6 relative grain overflow-hidden">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Daily brief</span>
            </div>
          ...
          {briefText ? (
            <p className="font-display text-xl md:text-2xl leading-relaxed text-foreground/90 fade-in">{briefText}</p>
          ) : (
            <p className="text-sm text-muted-foreground">Your quiet space is calm today. Generate a brief to see what matters now — pulled from your recent thoughts.</p>
          )}
          <div className="mt-6 flex flex-wrap gap-2">
            <Button asChild size="sm"><Link to="/app/workspace"><Pencil className="h-3.5 w-3.5 mr-1.5" /> Open workspace</Link></Button>
            <Button asChild size="sm" variant="outline"><Link to="/app/planner"><ListChecks className="h-3.5 w-3.5 mr-1.5" /> Planner</Link></Button>
            <Button asChild size="sm" variant="outline"><Link to="/app/calendar"><Calendar className="h-3.5 w-3.5 mr-1.5" /> Calendar</Link></Button>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-6xl px-4 md:px-10 pb-12">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/app/thoughts" className="text-xs text-muted-foreground hover:text-foreground inline-flex items-center">
        ...
        {thoughts.length === 0 ? (
          <div className="rounded-md border border-dashed border-border bg-card p-10 text-center">
            ...
            <Button asChild size="sm" className="mt-4"><Link to="/app/workspace">Transform your first thought</Link></Button>
          </div>
        ) : (
